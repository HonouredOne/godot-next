...
extends RefCounted
# author: KoBeWi (Modified by HonouredOne)
# license: MIT
# description:
#	A helper class for easier management and chaining of Tweens.
#	dynamically from code.
#
#	Currently, all of the custom Tweener classes are native classes,
#	so none of them are needed, but they are there just in case someone
